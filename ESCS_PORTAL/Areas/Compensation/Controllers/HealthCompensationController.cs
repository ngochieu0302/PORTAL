...
namespace ESCS.Areas.Compensation.Controllers
{
    [Area("Compensation")]
    [SystemAuthen]
    [ESCSDescription(ESCSMethod.GET, "Bồi thường con người")]
    public class HealthCompensationController : BaseController
    {
        ...
        public async Task<IActionResult> GetPaging()
        {
            ...
            var data = await Request.GetResponeNew(StoredProcedure.PORTAL_BH_BT_NG_TINH_TOAN_LKE, json);
            return Ok(data);
        }
        [AjaxOnly]
        ...
        {
            var json = Request.GetDataRequestNew(GetUser());
            var data = await Request.GetResponeNew(StoredProcedure.PORTAL_NG_TOAN_BO_THONG_TIN_HO_SO, json);
            return Ok(data);
        }
        [AjaxOnly]
