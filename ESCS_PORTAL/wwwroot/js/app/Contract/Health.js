...
//Select
var _selectNguoiLH = new SelectService("nguoi_lhla", true).getInstance().createInstance().setDataSource([
    { val: '', txt: 'Chọn mối quan hệ' },
    { val: 'BO', txt: 'Bố' },
    { val: 'ME', txt: 'Mẹ' },
    { val: 'BAN_THAN', txt: 'Bản thân' },
    { val: 'KHAC', txt: 'Khác (Anh/Chị/Em...)' },
], 'val', 'txt');
    ...
    _healthService.layDanhSachHopDong(data).then(res => {
        if (res.state_info.status === "OK") {
            ...
            const dshd_show = res.data_info.map((hd, i) => {
                return {
                    ...hd,
                    select_text: i == res.data_info.length - 1 ? 'Hợp đồng gốc' : `SĐBS lần ${res.data_info.length-1-i}`,
                };
            });
...
function onChangeXemThongTinHopDong(ma_doi_tac, ma_doi_tac_ql, ma_chi_nhanh_ql, so_id_d, so_id) {
    ...
    _healthService.getDetail(obj).then(res => {
        if (res.state_info.status === "OK") {
            ...
            $("#btnMoHoSoBT").hide();
            if (hop_dong_chi_tiet.hd.trang_thai == 'D') {
                $("#btnMoHoSoBT").show();
            }
...
function moHSBT() {
    if (_frmThongTinLienHe.isValid()) {
        ...
        obj.so_id_hd = hop_dong_chi_tiet.hd.so_id;
        obj.so_id_dt = $("#chiTietNDBH").find("input[name=so_id_dt]").val();
        obj.ngay_vv = obj.ngay_vv.split('/').reverse().join('');
        obj.ngay_rv = obj.ngay_rv.split('/').reverse().join('');
        obj.nguon = "MOBILE";
        //if (!checkForm(obj)) return;
        _healthService.moHoSoBT(obj).then(res => {
            if (res.state_info.status !== 'OK') {
...
$(document).ready(function () {
    ...
    $("#chkThamGiaLienHe").change(function () {
        ...
        if (_frmThongTinLienHe.getControlById("chkThamGiaLienHe").is(":checked")) {
            ...
            _selectNguoiLH.setValue("BAN_THAN");
            updateLabel();
    ...
    $("#chkThamGiaThongBao").change(function () {
        ...
        if (_frmThongTinLienHe.getControlById("chkThamGiaThongBao").is(":checked")) {
            ...
            _frmThongTinLienHe.getControl("email_nguoi_tb").val(customer.email);
